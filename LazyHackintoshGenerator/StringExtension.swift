...
extension String {
    ...
    func SysBuildVerBiggerThan(compare:String) -> Bool{
        ...
        if("0"..."9" ~= thisVerChars[thisVerChars.count-1]){
            thisVer.append(Int(String(thisVerChars[3..<(thisVerChars.count)]))!)
        }else{
        ...
        if("0"..."9" ~= compareVerChars[compareVerChars.count-1]){
            compareVer.append(Int(String(compareVerChars[3..<(compareVerChars.count)]))!)
        }else{
